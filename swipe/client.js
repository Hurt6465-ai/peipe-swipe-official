...
(function () {
  ...
  if (window.__peipePartnersSwipeV2) return;
  window.__peipePartnersSwipeV2 = true;
  ...
  function countryFlag(code) {
    var map = { CN: 'CN', MM: 'MM', VN: 'VN', VI: 'VN', TH: 'TH', US: 'US', GB: 'GB', UK: 'GB', JP: 'JP', JA: 'JP', KR: 'KR', KO: 'KR', EN: 'GB' };
    var c = map[String(code || '').toUpperCase()] || String(code || '').toUpperCase();
  ...
  function genderLabel(value) {
    ...
    if (value === 'male' || value === 'm' || value === '男') return TEXT.male;
    if (value === 'female' || value === 'f' || value === '女') return TEXT.female;
    if (value) return TEXT.other;
    return '';
  }

  function genderAge(user) {
    var parts = [];
    var gender = genderLabel(user.gender || user.genderCode);
  ...
  function isProfileComplete(profile) {
    return !!(profile && profile.displayName && profile.photos && profile.photos.length && profile.language_flag && profile.language_fluent && profile.language_learning && profile.gender && profile.birthday && profile.tags && profile.tags.length);
  }
  ...
  function iconGreet() {
    return '<svg viewBox="0 0 48 48" aria-hidden="true"><path d="M24 5l4.9 13.1L42 23l-13.1 4.9L24 41l-4.9-13.1L6 23l13.1-4.9L24 5z"></path></svg>';
  }
  ...
  function renderSlide(user, index) {
    ...
    return '' +
      '<section class="swiper-slide pps-slide" data-index="' + index + '" data-uid="' + Number(user.uid || 0) + '">' +
        ...
        '<div class="pps-top-actions">' +
          '<a href="/partners" class="pps-icon-btn" aria-label="' + escapeHtml(TEXT.back) + '">' + iconBack() + '</a>' +
          '<button type="button" class="pps-icon-btn pps-edit-profile" aria-label="' + escapeHtml(TEXT.settings) + '">' + iconSettings() + '</button>' +
        '</div>' +
        '<div class="pps-info">' +
          '<div class="pps-user-line">' + avatarHtml(user, 'pps-avatar') + '<div class="pps-name">' + escapeHtml(user.displayName || user.username || 'User') + '</div>' + (ga ? '<div class="pps-gender-age">' + escapeHtml(ga) + '</div>' : '') + (flag ? '<div class="pps-flag">' + escapeHtml(flag) + '</div>' : '') + '</div>' +
          '<div class="pps-lang-row"><span class="pps-chip">' + escapeHtml(native) + '</span><span class="pps-arrow">→</span><span class="pps-chip">' + escapeHtml(learn) + '</span></div>' +
          '<div class="pps-bio">' + escapeHtml(bio) + '</div>' +
          (tags ? '<div class="pps-tags">' + tags + '</div>' : '') +
        '</div>' +
        '<div class="pps-side-actions"><button type="button" class="pps-greet-btn" data-uid="' + Number(user.uid || 0) + '"><span class="pps-greet-icon">' + iconGreet() + '</span><span class="pps-greet-label">' + escapeHtml(TEXT.greet) + '</span></button></div>' +
      '</section>';
  }
  ...
  function buildSelect(name, list, value) {
    return '<select name="' + name + '">' + (list || []).map(function (item) {
  ...
  function validateProfile(data) {
    return data.displayName && data.photos.length && data.language_flag && data.language_fluent && data.language_learning && data.gender && data.birthday && data.tags.length;
  }

  function saveProfile() {
    if (state.uploadBusy) return toast(TEXT.uploading);
    var data = collectProfileData();
    if (!validateProfile(data)) return toast(TEXT.required);
  ...
  function uploadToNodeBB(file) {
    ...
    form.append('cid', String(CONFIG.uploadCid || 1));
