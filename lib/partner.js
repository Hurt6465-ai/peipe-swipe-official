...
const USER_FIELDS = [
  'uid', 'username', 'userslug', 'fullname', 'name', 'nickname', 'displayname', 'displayName', 'picture', 'uploadedpicture', 'status', 'lastonline', 'banned', 'deleted',
...
const PROFILE_FIELDS = [
  'uid', 'username', 'userslug', 'fullname', 'name', 'nickname', 'displayname', 'displayName', 'picture', 'uploadedpicture',
...
function publicDisplayName(data, uid) {
  const candidates = [
    data && data.peipe_partner_display_name,
    data && data.fullname,
    data && data.name,
    data && data.nickname,
  ...
  for (const candidate of candidates) {
    const text = cleanText(candidate, 80);
...
function decorateUser(data) {
  ...
  return {
    uid,
    username: publicDisplayName(data, uid) || cleanText(data.username || data.userslug || '', 80) || `User ${uid}`,
    displayName: publicDisplayName(data, uid) || cleanText(data.username || data.userslug || '', 80) || '未命名用户',
...
function missingFromProfile(profile) {
  const missing = [];
  if (!publicDisplayName(profile || {}, profile && profile.uid)) missing.push('displayName');
...
async function saveProfile(uid, body) {
  ...
  const displayName = cleanText(body.displayName || body.peipe_partner_display_name || current.peipe_partner_display_name || publicDisplayName(current || {}, uid) || '', 40);
  ...
  const occupationTag = tags.find(tag => /^occupation-/i.test(tag));
  const occupationFromTags = occupationTag ? cleanText(occupationTag.replace(/^occupation-/i, ''), 60) : '';
  ...
  const fields = {
    ...
    // 只同步到 fullname 和语伴私有字段；不要写 username/name/displayname/displayName，避免触发 NodeBB 用户名唯一性校验。
    ...
    peipe_partner_occupation: cleanText(body.occupation || body.job || occupationFromTags || current.peipe_partner_occupation || '', 60),
  };
