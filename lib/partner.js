...
const CONFIG = {
  maxScan: Math.max(100, Number(process.env.PEIPE_MAX_SCAN || 300) || 300),
  defaultLimit: Math.max(1, Number(process.env.PEIPE_DEFAULT_LIMIT || 18) || 18),
  maxLimit: Math.max(1, Number(process.env.PEIPE_MAX_LIMIT || 30) || 30),
  queueLimit: Math.max(30, Number(process.env.PEIPE_REC_QUEUE_SIZE || 300) || 300),
  seenTtlMs: 3 * 24 * 60 * 60 * 1000,
  dailyGreetLimit: 8,
...
const USER_FIELDS = [
  ...
  'relationship_status', 'lat', 'lng', 'peipe_partner_lat', 'peipe_partner_lng', 'peipe_partner_location_expires_at', 'languagePartnerGeoUpdatedAt', 'languagePartnerGeoExpiresAt',
...
const PROFILE_FIELDS = [
  ...
  'relationship_status', 'lat', 'lng', 'peipe_partner_lat', 'peipe_partner_lng', 'peipe_partner_location_expires_at', 'languagePartnerGeoExpiresAt',
  'peipe_partner_display_name', 'peipe_partner_photo', 'peipe_partner_photos', 'peipe_partner_tags',
  'peipe_partner_height', 'peipe_partner_weight', 'peipe_partner_education', 'peipe_partner_occupation',
  'peipe_partner_relationship',
];
...
function parseGeoRow(row) {
  ...
  const lat = Number(row.lat != null ? row.lat : row.peipe_partner_lat);
  const lng = Number(row.lng != null ? row.lng : row.peipe_partner_lng);
...
function jsonArrayString(value) {
  return JSON.stringify(toLangCodes(value).slice(0, 5));
}
...
function decorateUser(data) {
  ...
  const lat = Number(data.lat != null ? data.lat : data.peipe_partner_lat);
  const lng = Number(data.lng != null ? data.lng : data.peipe_partner_lng);
  ...
  return {
    ...
    relationshipStatus: cleanText(data.peipe_partner_relationship || data.relationship_status || '', 40),
    relationshipKey: cleanText(data.peipe_partner_relationship || data.relationship_status || '', 40),
    heightCm: cleanNumber(data.peipe_partner_height, 60, 260),
    weightKg: cleanNumber(data.peipe_partner_weight, 20, 300),
    education: cleanText(data.peipe_partner_education || '', 40),
    occupation: cleanText(data.peipe_partner_occupation || '', 60),
    relationshipEmoji: '',
    ...
    peipe_partner_location_expires_at: Number(data.peipe_partner_location_expires_at || data.languagePartnerGeoExpiresAt || 0) || 0,
    languagePartnerGeoExpiresAt: Number(data.languagePartnerGeoExpiresAt || data.peipe_partner_location_expires_at || 0) || 0,
...
async function getCandidateUids(mode) {
  const set = new Set();
  const add = (items) => {
    (items || []).forEach((uid) => {
      uid = Number(uid);
  ...
  if (String(mode || '').toLowerCase() === 'nearby') {
    add(await getRecentSortedSet('peipePartners:location:updated', CONFIG.maxScan));
    if (set.size) return Array.from(set).slice(0, CONFIG.maxScan);
  }
...
function scoreUser(viewer, item, seenSet, mode, viewerGeo) {
  let score = 0;
  ...
  if (String(mode || '').toLowerCase() !== 'nearby') {
    const km = roundedDistanceKm(distanceKm(viewerGeo, parseGeoRow(item)));
    if (km !== null && km <= RECOMMEND_NEARBY_PENALTY_RADIUS_KM) {
      score += RECOMMEND_NEARBY_PENALTY_SCORE;
    }
  }
  return score;
}
...
async function list(req) {
  ...
  const nearbyMode = String(mode || '').toLowerCase() === 'nearby';
  const maxLimit = req && req._peipeInternalQueue ? CONFIG.queueLimit : CONFIG.maxLimit;
  const limit = Math.min(maxLimit, Math.max(1, Number(req && req.query && req.query.limit) || CONFIG.defaultLimit));
  const seenSet = await getSeenSet(uid, mode);
  const candidateUids = (await getCandidateUids(mode)).filter(candidateUid => candidateUid !== uid);
  ...
  const viewerGeo = parseGeoRow(viewer);

  let ranked = (rows || []).map(row => decorateUser(row)).filter(Boolean);
  ...
  if (nearbyMode) {
    if (!viewerGeo) {
      ...
      ranked = ranked.map((item) => {
        const km = roundedDistanceKm(distanceKm(viewerGeo, parseGeoRow(item)));
        if (km === null || km > NEARBY_MAX_DISTANCE_KM) return null;
        item._distanceKm = km;
        item._score = scoreUser(viewer, item, seenSet, mode, viewerGeo);
        return item;
    ...
    ranked = ranked
      .map(item => Object.assign(item, { _score: scoreUser(viewer, item, seenSet, mode, viewerGeo) }))
      .sort((a, b) => b._score - a._score);
  }

  const total = ranked.length;
  const users = ranked.slice(0, limit).map((item) => {
    delete item._score;
    delete item._distanceKm;
    return item;
  });

  if (!(req && req._peipeSkipMarkSeen)) {
    await markSeen(uid, mode, users);
  }
  return { ok: true, users, hasMore: total > limit, mode, limit };
}

async function getUsersByUids(uids) {
  const ids = Array.from(new Set((uids || []).map(Number).filter(Boolean))).slice(0, CONFIG.queueLimit);
  if (!ids.length) return [];
  const rows = await user.getUsersFields(ids, USER_FIELDS).catch(() => []);
  return (rows || []).map(row => decorateUser(row)).filter(Boolean);
}
...
function missingFromProfile(profile) {
  ...
  if (!toLangCodes(profile.language_fluent).length) missing.push('language_fluent');
  if (!toLangCodes(profile.language_learning).length) missing.push('language_learning');
...
async function profileStatus(uid) {
  let profile = await user.getUserFields(uid, PROFILE_FIELDS);
  profile = await repairJsonLanguageFields(uid, profile || {});
...
async function saveProfile(uid, body) {
  body = body || {};
  let current = await user.getUserFields(uid, PROFILE_FIELDS);
  ...
  const fields = {
    ...
    age,
    relationship_status: cleanText(body.relationship_status || body.relationship || current.relationship_status || '', 40),
...
async function saveLocation(uid, body) {
  ...
  // 默认保存 7 天。后续用户重新授权/上传位置时直接覆盖旧坐标。
  const ttlDays = Math.max(1, Number(process.env.PEIPE_LOCATION_TTL_DAYS || 7) || 7);
  const timestamp = now();
  ...
  await user.setUserFields(uid, {
    ...
    peipe_partner_location_updated_at: timestamp,
    peipe_partner_location_expires_at: expiresAt,
    languagePartnerGeoUpdatedAt: timestamp,
    languagePartnerGeoExpiresAt: expiresAt,
  });
  if (typeof db.sortedSetAdd === 'function') {
    await db.sortedSetAdd('peipePartners:location:updated', timestamp, String(uid)).catch(() => {});
  }
  return { ok: true, lat, lng, accuracy, updatedAt: timestamp, expiresAt };
}
