...
const USER_FIELDS = [
  'uid', 'username', 'userslug', 'picture', 'uploadedpicture', 'status', 'lastonline',
  'banned', 'deleted', 'aboutme', 'signature', 'bio',
  ...
  'gender', 'sex', 'age', 'relationship_status', 'lat', 'lng',
  'languagePartnerGeoUpdatedAt', 'languagePartnerGeoExpiresAt',
  'peipe_partner_display_name', 'peipe_partner_photo', 'peipe_partner_photos', 'peipe_partner_tags',
];

const PROFILE_FIELDS = [
  'uid', 'username', 'picture', 'uploadedpicture', 'bio', 'aboutme', 'signature',
  'language_flag', 'language_fluent', 'language_learning', 'gender', 'age', 'relationship_status',
  'peipe_partner_display_name', 'peipe_partner_photo', 'peipe_partner_photos', 'peipe_partner_tags',
];
...
function decorateUser(data) {
  ...
  if (!uid || data.banned || data.deleted) {
    return null;
  }
  ...
  const mainPhoto = cleanUrl(data.peipe_partner_photo || accountPicture);
  const photos = normalisePhotos(data.peipe_partner_photos, mainPhoto || accountPicture);
  const rawBio = cleanText(data.bio || data.aboutme || data.signature || '', 180);
  ...
  return {
    ...
    photos,
    tags,
    gender: cleanText(data.gender || data.sex || '', 40),
    genderCode: cleanText(data.gender || data.sex || '', 40),
    age: Number(data.age || 0) || 0,
    ageText: Number(data.age || 0) ? `${Number(data.age)}岁` : '',
...
async function getSortedSet(key, limit) {
  try {
    if (typeof db.getSortedSetRevRange === 'function') {
      return await db.getSortedSetRevRange(key, 0, limit - 1);
    }
  } catch (err) {
    // ignore missing sorted set
  }
  return [];
}
...
async function getRecentSortedSet(key, limit) {
  try {
    if (typeof db.getSortedSetRevRangeByScore === 'function') {
      return await db.getSortedSetRevRangeByScore(key, 0, limit - 1, '+inf', 0);
    }
  } catch (err) {
    // ignore missing sorted set
  }
...
async function markSeen(uid, mode, users) {
  if (!uid || !users.length || typeof db.sortedSetAdd !== 'function') {
    return;
  }
  const key = `peipePartners:seen:${mode || 'recommend'}:${uid}`;
  const score = now();
...
async function list(req) {
  ...
  if (!candidateUids.length) {
    return { ok: true, users: [], hasMore: false, mode, limit };
  }
...
function missingFromProfile(profile) {
  ...
  if (!cleanUrl(profile.peipe_partner_photo || profile.picture || profile.uploadedpicture)) missing.push('picture');
  if (!cleanText(profile.language_flag)) missing.push('language_flag');
  if (!cleanText(profile.language_fluent)) missing.push('language_fluent');
  if (!cleanText(profile.language_learning)) missing.push('language_learning');
  if (!cleanText(profile.gender)) missing.push('gender');
  if (!Number(profile.age || 0)) missing.push('age');
  return missing;
}
...
async function saveProfile(uid, body) {
  ...
  const fields = {
    ...
    bio: cleanText(body.bio || current.bio || current.aboutme || current.signature || '', 180),
...
async function isVip(uid) {
  if (!uid || !groups || typeof groups.isMemberOfGroups !== 'function') {
    return false;
  }
  try {
...
async function greet(uid, body) {
  ...
  if (count >= limit) {
    return { ok: false, error: 'daily-limit', limit, count };
  }

  await db.set(key, count + 1).catch(() => {});
  if (typeof db.expire === 'function') {
    await db.expire(key, 36 * 60 * 60).catch(() => {});
  }

  const message = cleanText(body.message || '你好，可以一起练语言吗？', 120);
  const payload = JSON.stringify({ fromUid: Number(uid), toUid: targetUid, message, createdAt: now() });
  await Promise.all([
  ...
  return { ok: true, remaining: Math.max(0, limit - count - 1), message };
}
